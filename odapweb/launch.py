...
class launchInstanceThread(threading.Thread):
	"""
	This thread contains the launch logic for a may instance per ODAP-157. It will update
	...
	def launch_instance(self, form):
		...
		try:
			...
			cf_response = conn.create_stack(
				StackName="AWSLXMliy-" + lgroup.ags + str(uuid.uuid4()),  # TODO Add UUID?
	...
	def provide_instance_details(self, instance_id, swconfig, logger, itype, lgroup, s_rec):
		...
		if created:
			...
			try:
				...
				logger.debug("DNS Status Code:" + str(r.status_code))
				...
				irec.dns_url = dnsDisplayName(irec.instance_id)
