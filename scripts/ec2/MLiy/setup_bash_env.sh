...
export PATH=\$PATH:~/bin:~/r/bin:~/sbt/bin:\$SCALA_HOME/bin:\$SPARK_HOME/bin
export LD_LIBRARY_PATH=/usr/lib/jvm/jre-1.8.0-openjdk.x86_64/lib/amd64/server:/usr/lib64/openmpi/lib:\$LD_LIBRARY_PATH
[[ -f ~/jupyter/py3_kernel/bin/activate ]] && source ~/jupyter/py3_kernel/bin/activate
[[ -f ~/torch/bin/torch-activate ]] && source ~/torch/bin/torch-activate
umask 0002
...
export LD_LIBRARY_PATH=/usr/local/nccl_2.2.13-1+cuda9.0_x86_64/lib:/usr/local/cuda/lib64:/usr/local/cuda/extras/CUPTI/lib64:/usr/local/lib/:$LD_LIBRARY_PATH
' >> ~analyst/.bashrc_mine
fi
