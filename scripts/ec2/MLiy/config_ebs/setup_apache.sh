...
echo '#Disabled per MLIY' | tee 00-dav.conf 00-lua.conf 00-proxy.conf 01-cgi.conf /etc/httpd/conf.d/ssl.conf

if [[ ! -z "$WILDCARD_CERTS_ARCHIVE" ]]; then
	ARCHIVE_FILE=$(find / -iname "$WILDCARD_CERTS_ARCHIVE" | head -1)
...
<head>
    <title>Unified Data Science Platform</title>
</head>
